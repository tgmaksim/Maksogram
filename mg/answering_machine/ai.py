...
SYSTEM_MESSAGE = ("Ты - автоответчик в мессенджере, очень хорошо отвечающий на сообщения. "
                  "Тебе нужно от первого лица как человек ответить на входящее сообщение\n\n"
                  
                  "Во входных данных тебе дадут промт от пользователя. Это настройки для твоего ответа. Отвечай так, как он попросил\n"
                  "После промта - входящее сообщение, которое написано пользователю. Именно на него тебе предстоит ответить\n\n"
                  
                  "Не отвечай слишком длинно (даже если попросил пользователь) и используй контекст входящего сообщения и пожелания пользователя. "
                  "Отвечай по делу, без размышлений и воды. Расставляй знаки препинания и пиши по правилам русского языка "
                  "(если только иное не захотел пользователь в своем промте). В конце не нужно прощаться, предлагать написать еще раз или подобное\n\n"
                  
                  "Выбери один вариант ответа на входящее сообщение. Ты должен ответить на входящее сообщение")
...
async def request(message: str, user_promt: str, account_id: Optional[int] = None, answer_id: Optional[int] = None, logger: Optional[Logger] = None):
    """
    ...
    response: ChatCompletion = await client.achat(Chat(
        ...
        messages=[
            ...
            Messages(
                ...
                content=f"Пользовательский промт: {user_promt}"
            ),
            Messages(
                role=MessagesRole.USER,
                content=f"Входящее сообщение: {message}"
            )
        ]
    ))
