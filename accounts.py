...
class Account:
    ...
    def get_accounts() -> list['Account']:
        with connect(core.resources_path("db.sqlite3")) as conn:
            cur = conn.cursor()
            cur.execute("SELECT name, id, password, phone, device_model, my_messages, message_changes, "
                        "channel_status_username, message_status_id, add_chats, status_users FROM accounts")
            accounts = cur.fetchall()
        return [Account(d0, d1, d2, d3, d4, d5, d6, d7, d8, json().decode(d9), json().decode(d10))
                for d0, d1, d2, d3, d4, d5, d6, d7, d8, d9, d10 in accounts]
