...
def ghost_mode_menu() -> dict[str, Any]:
...
async def _ghost_stories(callback_query: CallbackQuery, state: FSMContext):
    ...
    message_id = (await callback_query.message.answer("Отправьте пользователя для просмотра истории", reply_markup=markup)).message_id
